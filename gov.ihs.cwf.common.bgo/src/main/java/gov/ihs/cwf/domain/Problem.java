...
public class Problem extends DomainObject {
    ...
    public Problem(String value) {
        ...
        numberCode = pcs[0];
        patient = new Patient(Long.parseLong(pcs[1]));
        icd9Code = new ICD9Concept();
        icd9Code.setCode(pcs[2]);
        icd9Code.setDomainId(NumberUtils.toLong(pcs[11]));
        ...
        setDomainId(Long.parseLong(pcs[9]));
        notes = pcs[10];
        ...
        if (!pcs[14].isEmpty()) {
            facility = new Institution(Long.parseLong(pcs[14]));
        }
